...
{
    if (m_options->colors()) {
        LOG_INFO("\x1B[01;35mnew job\x1B[0m from \x1B[01;37m%s:%d\x1B[0m diff \x1B[01;37m%" PRIu64, client->host(), client->port(), job.diff());
    }
    else {
        LOG_INFO("new job from %s:%d diff %" PRIu64, client->host(), client->port(), job.diff());
    }
